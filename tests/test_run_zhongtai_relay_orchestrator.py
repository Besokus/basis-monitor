...
class RunZhongtaiRelayOrchestratorTest(unittest.TestCase):
    ...
    def test_is_time_within_report_window_is_inclusive(self):
        ...
        self.assertTrue(self.module.is_time_within_report_window(now.replace(hour=14, minute=55), "15:00", 5, 5))
        self.assertFalse(self.module.is_time_within_report_window(now.replace(hour=14, minute=54), "15:00", 5, 5))

    def test_main_triggers_report_relay_within_window(self):
        calls = []

        def fake_pull_outputs(*args, **kwargs):
            include_report_texts = kwargs.get("include_report_texts")
            calls.append(f"pull:{include_report_texts}")
        ...
        self.assertEqual(["pull:True", "alerts", "report"], calls)

    def test_main_skips_report_relay_outside_window(self):
        calls = []
        ...
        self.assertEqual(0, exit_code)
        self.assertEqual(["pull:False", "alerts"], calls)
