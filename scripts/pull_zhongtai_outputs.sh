...
if [ -n "$SFTP_IDENTITY_FILE" ]; then
    echo "Downloading through SFTP: target=$REMOTE_TARGET port=$SFTP_PORT identity=$SFTP_IDENTITY_FILE"
    sftp -P "$SFTP_PORT" -i "$SFTP_IDENTITY_FILE" -b "$BATCH_FILE" "$REMOTE_TARGET"
else
    echo "Downloading through SFTP: target=$REMOTE_TARGET port=$SFTP_PORT"
    sftp -P "$SFTP_PORT" -b "$BATCH_FILE" "$REMOTE_TARGET"
fi

echo "Pulled Zhongtai outputs into $LOCAL_OUTPUT_ROOT"
