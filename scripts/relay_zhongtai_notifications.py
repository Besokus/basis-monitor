...
def record_report_receipts(report_paths: List[pathlib.Path], state: Dict[str, object]) -> None:
    ...
    for report_path in report_paths:
        ...
        print(f"[REPORT_RECEIPT] {report_path.name} received_at={now_text}")
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if attempt < max_retries:
                print(f"[WECOM_RETRY] attempt={attempt}/{max_retries} reason={exc}",
                      file=sys.stderr)
...
def relay_alerts(webhook: str, csv_path: pathlib.Path, state: Dict[str, object],
    ...
    for row in rows:
        ...
        try:
            ...
            print(f"[WECOM_ALERT_FAILED] key={unique_key} reason={exc}", file=sys.stderr)
            continue
        sent_alert_keys.add(unique_key)
        relayed += 1
        print(f"[WECOM_ALERT_SENT] key={unique_key}")
...
def relay_reports(webhook: str, report_dir: pathlib.Path, state: Dict[str, object],
    ...
    for report_path in report_paths:
        ...
        if attempts >= max_report_send_attempts:
            print(f"[WECOM_REPORT_GAVEUP] {report_path.name} attempts={attempts}",
                  file=sys.stderr)
        ...
        try:
            ...
            print(f"[WECOM_RELAY_FAILED] {report_path.name} attempts={send_attempts[report_path.name]} reason={exc}",
                  file=sys.stderr)
            continue
        sent_reports.add(report_path.name)
        send_attempts.pop(report_path.name, None)
        relayed += 1
        print(f"[WECOM_REPORT_SENT] {report_path.name}")
...
def main() -> int:
    ...
    if not webhook:
        print("Missing webhook. Use --webhook or WECOM_ROBOT_WEBHOOK.", file=sys.stderr)
        return 2
    ...
    if not args.skip_alerts:
        try:
            ...
            print(f"[RELAY_ALERTS_CRASH] {exc}", file=sys.stderr)
    ...
    if not args.skip_reports:
        try:
            ...
            print(f"[RELAY_REPORTS_CRASH] {exc}", file=sys.stderr)

    save_state(state_path, state)
    print(f"Relayed alerts: {relayed_alerts}")
    print(f"Relayed reports: {relayed_reports}")
    return 0
