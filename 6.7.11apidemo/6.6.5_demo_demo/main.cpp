...
int main()
{
	...
	while (true)
	{
		...
		case 2:
		{
			...
			md_InstrumentID.push_back("IO2509-C-4700");
		...
		case 1:
		{
			...
			pUserApi->CreateFtdcTraderApi(".\\flow\\",true);
		...
		cir:pUserApi->RegisterSpi(&sh);
			...
			memset(&g_chpFrontInfo, 0, sizeof(CThostFtdcFrontInfoField));
			...
			loop:int input1;
				...
				case 110:
				{
					...
					CThostFtdcMdApi  *pUserMdApi =
						CThostFtdcMdApi::CreateFtdcMdApi(".\\flow\\");
					CSimpleMdHandler ash(pUserMdApi);
					pUserMdApi->RegisterSpi(&ash);
					pUserMdApi->RegisterFront(const_cast<char *>(g_chFrontMdaddr.c_str()));
					pUserMdApi->Init();
				...
				search:system("cls");
					...
					case 45://请求查询交易所调整保证金率
					{
						sh.ReqQryRiskSettleInvstPosition();
						_getch();
						break;
					}
					case 46://请求查询交易所调整保证金率
					{
