...
CThostFtdcTraderApi* CTraderApi::CreateFtdcTraderApi(const char *pszFlowPath /*= ""*/ , bool bIsProductionMode)
{
	return m_pApi = CThostFtdcTraderApi::CreateFtdcTraderApi(pszFlowPath, bIsProductionMode);
}
...
void CTraderApi::GetFrontInfo(CThostFtdcFrontInfoField* pFrontInfo)
{
	LOG("<GetFrontInfo>\n");
	if (pFrontInfo)
	{
	...
	m_pApi->GetFrontInfo(pFrontInfo);
};
...
int CTraderApi::RegisterWechatUserSystemInfo(CThostFtdcWechatUserSystemInfoField* pUserSystemInfo)
{
	LOG("<RegisterWechatUserSystemInfo>\n");
	if (pUserSystemInfo)
	{
		LOG("\tBrokerID [%s]\n", pUserSystemInfo->BrokerID);
		LOG("\tUserID [%s]\n", pUserSystemInfo->UserID);
		LOG("\tWechatCltSysInfo [%s]\n", pUserSystemInfo->WechatCltSysInfo);
		LOG("\tClientLoginTime [%s]\n", pUserSystemInfo->ClientLoginTime);
		LOG("\tClientAppID [%s]\n", pUserSystemInfo->ClientAppID);
		LOG("\tClientPublicIP [%s]\n", pUserSystemInfo->ClientPublicIP);
		LOG("\tClientLoginRemark [%s]\n", pUserSystemInfo->ClientLoginRemark);
		LOG("\tWechatCltSysInfoLen [%d]\n", pUserSystemInfo->WechatCltSysInfoLen);
		LOG("\tClientIPPort [%d]\n", pUserSystemInfo->ClientIPPort);
	}
	LOG("</RegisterWechatUserSystemInfo>\n");

	return m_pApi->RegisterWechatUserSystemInfo(pUserSystemInfo);
};

int CTraderApi::SubmitWechatUserSystemInfo(CThostFtdcWechatUserSystemInfoField* pUserSystemInfo)
{
	LOG("<SubmitWechatUserSystemInfo>\n");
	if (pUserSystemInfo)
	{
		LOG("\tBrokerID [%s]\n", pUserSystemInfo->BrokerID);
		LOG("\tUserID [%s]\n", pUserSystemInfo->UserID);
	...
	return m_pApi->SubmitWechatUserSystemInfo(pUserSystemInfo);
};
...
int CTraderApi::ReqQryUserSession(CThostFtdcQryUserSessionField* pQryUserSession, int nRequestID)
{
	LOG("<ReqQryUserSession>\n");
	if (pQryUserSession)
	{
		LOG("\tBrokerID [%s]\n", pQryUserSession->BrokerID);
		LOG("\tUserID [%s]\n", pQryUserSession->UserID);
		LOG("\tFrontID [%d]\n", pQryUserSession->FrontID);
		LOG("\tSessionID [%d]\n", pQryUserSession->SessionID);
	}
	LOG("\tnRequestID [%d]\n", nRequestID);
	LOG("</ReqQryUserSession>\n");

	return m_pApi->ReqQryUserSession(pQryUserSession, nRequestID);
};
...
int CTraderApi::ReqQryDepthMarketData(CThostFtdcQryDepthMarketDataField* pQryDepthMarketData, int nRequestID)
{
	...
	if (pQryDepthMarketData)
	{
		...
		LOG("\tProductClass [%c]\n", pQryDepthMarketData->ProductClass);
	}
	LOG("\tnRequestID [%d]\n", nRequestID);
	LOG("</ReqQryDepthMarketData>\n");
...
int CTraderApi::ReqQrySPBMFutureParameter(CThostFtdcQrySPBMFutureParameterField* pQrySPBMFutureParameter, int nRequestID)
{
	LOG("<ReqQrySPBMFutureParameter>\n");
	if (pQrySPBMFutureParameter)
	{
		LOG("\tExchangeID [%s]\n", pQrySPBMFutureParameter->ExchangeID);
		LOG("\tInstrumentID [%s]\n", pQrySPBMFutureParameter->InstrumentID);
		LOG("\tProdFamilyCode [%s]\n", pQrySPBMFutureParameter->ProdFamilyCode);
	}
	LOG("\tnRequestID [%d]\n", nRequestID);
	LOG("</ReqQrySPBMFutureParameter>\n");

	return m_pApi->ReqQrySPBMFutureParameter(pQrySPBMFutureParameter, nRequestID);
};


int CTraderApi::ReqQrySPBMOptionParameter(CThostFtdcQrySPBMOptionParameterField* pQrySPBMOptionParameter, int nRequestID)
{
	LOG("<ReqQrySPBMOptionParameter>\n");
	if (pQrySPBMOptionParameter)
	{
		LOG("\tExchangeID [%s]\n", pQrySPBMOptionParameter->ExchangeID);
		LOG("\tInstrumentID [%s]\n", pQrySPBMOptionParameter->InstrumentID);
		LOG("\tProdFamilyCode [%s]\n", pQrySPBMOptionParameter->ProdFamilyCode);
	}
	LOG("\tnRequestID [%d]\n", nRequestID);
	LOG("</ReqQrySPBMOptionParameter>\n");

	return m_pApi->ReqQrySPBMOptionParameter(pQrySPBMOptionParameter, nRequestID);
};


int CTraderApi::ReqQrySPBMIntraParameter(CThostFtdcQrySPBMIntraParameterField* pQrySPBMIntraParameter, int nRequestID)
{
	LOG("<ReqQrySPBMIntraParameter>\n");
	if (pQrySPBMIntraParameter)
	{
		LOG("\tExchangeID [%s]\n", pQrySPBMIntraParameter->ExchangeID);
		LOG("\tProdFamilyCode [%s]\n", pQrySPBMIntraParameter->ProdFamilyCode);
	}
	LOG("\tnRequestID [%d]\n", nRequestID);
	LOG("</ReqQrySPBMIntraParameter>\n");

	return m_pApi->ReqQrySPBMIntraParameter(pQrySPBMIntraParameter, nRequestID);
};


int CTraderApi::ReqQrySPBMInterParameter(CThostFtdcQrySPBMInterParameterField* pQrySPBMInterParameter, int nRequestID)
{
	LOG("<ReqQrySPBMInterParameter>\n");
	if (pQrySPBMInterParameter)
	{
		LOG("\tExchangeID [%s]\n", pQrySPBMInterParameter->ExchangeID);
		LOG("\tLeg1ProdFamilyCode [%s]\n", pQrySPBMInterParameter->Leg1ProdFamilyCode);
		LOG("\tLeg2ProdFamilyCode [%s]\n", pQrySPBMInterParameter->Leg2ProdFamilyCode);
	}
	LOG("\tnRequestID [%d]\n", nRequestID);
	LOG("</ReqQrySPBMInterParameter>\n");

	return m_pApi->ReqQrySPBMInterParameter(pQrySPBMInterParameter, nRequestID);
};


int CTraderApi::ReqQrySPBMPortfDefinition(CThostFtdcQrySPBMPortfDefinitionField* pQrySPBMPortfDefinition, int nRequestID)
{
	LOG("<ReqQrySPBMPortfDefinition>\n");
	if (pQrySPBMPortfDefinition)
	{
		LOG("\tExchangeID [%s]\n", pQrySPBMPortfDefinition->ExchangeID);
		LOG("\tProdFamilyCode [%s]\n", pQrySPBMPortfDefinition->ProdFamilyCode);
		LOG("\tPortfolioDefID [%d]\n", pQrySPBMPortfDefinition->PortfolioDefID);
	}
	LOG("\tnRequestID [%d]\n", nRequestID);
	LOG("</ReqQrySPBMPortfDefinition>\n");

	return m_pApi->ReqQrySPBMPortfDefinition(pQrySPBMPortfDefinition, nRequestID);
};


int CTraderApi::ReqQrySPBMInvestorPortfDef(CThostFtdcQrySPBMInvestorPortfDefField* pQrySPBMInvestorPortfDef, int nRequestID)
{
	LOG("<ReqQrySPBMInvestorPortfDef>\n");
	if (pQrySPBMInvestorPortfDef)
	{
		LOG("\tExchangeID [%s]\n", pQrySPBMInvestorPortfDef->ExchangeID);
		LOG("\tBrokerID [%s]\n", pQrySPBMInvestorPortfDef->BrokerID);
		LOG("\tInvestorID [%s]\n", pQrySPBMInvestorPortfDef->InvestorID);
	}
	LOG("\tnRequestID [%d]\n", nRequestID);
	LOG("</ReqQrySPBMInvestorPortfDef>\n");

	return m_pApi->ReqQrySPBMInvestorPortfDef(pQrySPBMInvestorPortfDef, nRequestID);
};


int CTraderApi::ReqQryInvestorPortfMarginRatio(CThostFtdcQryInvestorPortfMarginRatioField* pQryInvestorPortfMarginRatio, int nRequestID)
{
	LOG("<ReqQryInvestorPortfMarginRatio>\n");
	if (pQryInvestorPortfMarginRatio)
	{
		LOG("\tBrokerID [%s]\n", pQryInvestorPortfMarginRatio->BrokerID);
		LOG("\tInvestorID [%s]\n", pQryInvestorPortfMarginRatio->InvestorID);
		LOG("\tExchangeID [%s]\n", pQryInvestorPortfMarginRatio->ExchangeID);
		LOG("\tProductGroupID [%s]\n", pQryInvestorPortfMarginRatio->ProductGroupID);
	}
	LOG("\tnRequestID [%d]\n", nRequestID);
	LOG("</ReqQryInvestorPortfMarginRatio>\n");

	return m_pApi->ReqQryInvestorPortfMarginRatio(pQryInvestorPortfMarginRatio, nRequestID);
};


int CTraderApi::ReqQryInvestorProdSPBMDetail(CThostFtdcQryInvestorProdSPBMDetailField* pQryInvestorProdSPBMDetail, int nRequestID)
{
	LOG("<ReqQryInvestorProdSPBMDetail>\n");
	if (pQryInvestorProdSPBMDetail)
	{
		LOG("\tExchangeID [%s]\n", pQryInvestorProdSPBMDetail->ExchangeID);
		LOG("\tBrokerID [%s]\n", pQryInvestorProdSPBMDetail->BrokerID);
		LOG("\tInvestorID [%s]\n", pQryInvestorProdSPBMDetail->InvestorID);
		LOG("\tProdFamilyCode [%s]\n", pQryInvestorProdSPBMDetail->ProdFamilyCode);
	}
	LOG("\tnRequestID [%d]\n", nRequestID);
	LOG("</ReqQryInvestorProdSPBMDetail>\n");

	return m_pApi->ReqQryInvestorProdSPBMDetail(pQryInvestorProdSPBMDetail, nRequestID);
};


int CTraderApi::ReqQryInvestorCommoditySPMMMargin(CThostFtdcQryInvestorCommoditySPMMMarginField* pQryInvestorCommoditySPMMMargin, int nRequestID)
{
	LOG("<ReqQryInvestorCommoditySPMMMargin>\n");
	if (pQryInvestorCommoditySPMMMargin)
	{
		LOG("\tBrokerID [%s]\n", pQryInvestorCommoditySPMMMargin->BrokerID);
		LOG("\tInvestorID [%s]\n", pQryInvestorCommoditySPMMMargin->InvestorID);
		LOG("\tCommodityID [%s]\n", pQryInvestorCommoditySPMMMargin->CommodityID);
	}
	LOG("\tnRequestID [%d]\n", nRequestID);
	LOG("</ReqQryInvestorCommoditySPMMMargin>\n");

	return m_pApi->ReqQryInvestorCommoditySPMMMargin(pQryInvestorCommoditySPMMMargin, nRequestID);
};


int CTraderApi::ReqQryInvestorCommodityGroupSPMMMargin(CThostFtdcQryInvestorCommodityGroupSPMMMarginField* pQryInvestorCommodityGroupSPMMMargin, int nRequestID)
{
	LOG("<ReqQryInvestorCommodityGroupSPMMMargin>\n");
	if (pQryInvestorCommodityGroupSPMMMargin)
	{
		LOG("\tBrokerID [%s]\n", pQryInvestorCommodityGroupSPMMMargin->BrokerID);
		LOG("\tInvestorID [%s]\n", pQryInvestorCommodityGroupSPMMMargin->InvestorID);
		LOG("\tCommodityGroupID [%s]\n", pQryInvestorCommodityGroupSPMMMargin->CommodityGroupID);
	}
	LOG("\tnRequestID [%d]\n", nRequestID);
	LOG("</ReqQryInvestorCommodityGroupSPMMMargin>\n");

	return m_pApi->ReqQryInvestorCommodityGroupSPMMMargin(pQryInvestorCommodityGroupSPMMMargin, nRequestID);
};


int CTraderApi::ReqQrySPMMInstParam(CThostFtdcQrySPMMInstParamField* pQrySPMMInstParam, int nRequestID)
{
	LOG("<ReqQrySPMMInstParam>\n");
	if (pQrySPMMInstParam)
	{
		LOG("\tInstrumentID [%s]\n", pQrySPMMInstParam->InstrumentID);
	}
	LOG("\tnRequestID [%d]\n", nRequestID);
	LOG("</ReqQrySPMMInstParam>\n");

	return m_pApi->ReqQrySPMMInstParam(pQrySPMMInstParam, nRequestID);
};


int CTraderApi::ReqQrySPMMProductParam(CThostFtdcQrySPMMProductParamField* pQrySPMMProductParam, int nRequestID)
{
	LOG("<ReqQrySPMMProductParam>\n");
	if (pQrySPMMProductParam)
	{
		LOG("\tProductID [%s]\n", pQrySPMMProductParam->ProductID);
	}
	LOG("\tnRequestID [%d]\n", nRequestID);
	LOG("</ReqQrySPMMProductParam>\n");

	return m_pApi->ReqQrySPMMProductParam(pQrySPMMProductParam, nRequestID);
};


int CTraderApi::ReqQrySPBMAddOnInterParameter(CThostFtdcQrySPBMAddOnInterParameterField* pQrySPBMAddOnInterParameter, int nRequestID)
{
	LOG("<ReqQrySPBMAddOnInterParameter>\n");
	if (pQrySPBMAddOnInterParameter)
	{
		LOG("\tExchangeID [%s]\n", pQrySPBMAddOnInterParameter->ExchangeID);
		LOG("\tLeg1ProdFamilyCode [%s]\n", pQrySPBMAddOnInterParameter->Leg1ProdFamilyCode);
		LOG("\tLeg2ProdFamilyCode [%s]\n", pQrySPBMAddOnInterParameter->Leg2ProdFamilyCode);
	}
	LOG("\tnRequestID [%d]\n", nRequestID);
	LOG("</ReqQrySPBMAddOnInterParameter>\n");

	return m_pApi->ReqQrySPBMAddOnInterParameter(pQrySPBMAddOnInterParameter, nRequestID);
};


int CTraderApi::ReqQryRCAMSCombProductInfo(CThostFtdcQryRCAMSCombProductInfoField* pQryRCAMSCombProductInfo, int nRequestID)
{
	LOG("<ReqQryRCAMSCombProductInfo>\n");
	if (pQryRCAMSCombProductInfo)
	{
		LOG("\tProductID [%s]\n", pQryRCAMSCombProductInfo->ProductID);
		LOG("\tCombProductID [%s]\n", pQryRCAMSCombProductInfo->CombProductID);
		LOG("\tProductGroupID [%s]\n", pQryRCAMSCombProductInfo->ProductGroupID);
	}
	LOG("\tnRequestID [%d]\n", nRequestID);
	LOG("</ReqQryRCAMSCombProductInfo>\n");

	return m_pApi->ReqQryRCAMSCombProductInfo(pQryRCAMSCombProductInfo, nRequestID);
};


int CTraderApi::ReqQryRCAMSInstrParameter(CThostFtdcQryRCAMSInstrParameterField* pQryRCAMSInstrParameter, int nRequestID)
{
	LOG("<ReqQryRCAMSInstrParameter>\n");
	if (pQryRCAMSInstrParameter)
	{
		LOG("\tProductID [%s]\n", pQryRCAMSInstrParameter->ProductID);
	}
	LOG("\tnRequestID [%d]\n", nRequestID);
	LOG("</ReqQryRCAMSInstrParameter>\n");

	return m_pApi->ReqQryRCAMSInstrParameter(pQryRCAMSInstrParameter, nRequestID);
};


int CTraderApi::ReqQryRCAMSIntraParameter(CThostFtdcQryRCAMSIntraParameterField* pQryRCAMSIntraParameter, int nRequestID)
{
	LOG("<ReqQryRCAMSIntraParameter>\n");
	if (pQryRCAMSIntraParameter)
	{
		LOG("\tCombProductID [%s]\n", pQryRCAMSIntraParameter->CombProductID);
	}
	LOG("\tnRequestID [%d]\n", nRequestID);
	LOG("</ReqQryRCAMSIntraParameter>\n");

	return m_pApi->ReqQryRCAMSIntraParameter(pQryRCAMSIntraParameter, nRequestID);
};


int CTraderApi::ReqQryRCAMSInterParameter(CThostFtdcQryRCAMSInterParameterField* pQryRCAMSInterParameter, int nRequestID)
{
	LOG("<ReqQryRCAMSInterParameter>\n");
	if (pQryRCAMSInterParameter)
	{
		LOG("\tProductGroupID [%s]\n", pQryRCAMSInterParameter->ProductGroupID);
		LOG("\tCombProduct1 [%s]\n", pQryRCAMSInterParameter->CombProduct1);
		LOG("\tCombProduct2 [%s]\n", pQryRCAMSInterParameter->CombProduct2);
	}
	LOG("\tnRequestID [%d]\n", nRequestID);
	LOG("</ReqQryRCAMSInterParameter>\n");

	return m_pApi->ReqQryRCAMSInterParameter(pQryRCAMSInterParameter, nRequestID);
};


int CTraderApi::ReqQryRCAMSShortOptAdjustParam(CThostFtdcQryRCAMSShortOptAdjustParamField* pQryRCAMSShortOptAdjustParam, int nRequestID)
{
	LOG("<ReqQryRCAMSShortOptAdjustParam>\n");
	if (pQryRCAMSShortOptAdjustParam)
	{
		LOG("\tCombProductID [%s]\n", pQryRCAMSShortOptAdjustParam->CombProductID);
	}
	LOG("\tnRequestID [%d]\n", nRequestID);
	LOG("</ReqQryRCAMSShortOptAdjustParam>\n");

	return m_pApi->ReqQryRCAMSShortOptAdjustParam(pQryRCAMSShortOptAdjustParam, nRequestID);
};


int CTraderApi::ReqQryRCAMSInvestorCombPosition(CThostFtdcQryRCAMSInvestorCombPositionField* pQryRCAMSInvestorCombPosition, int nRequestID)
{
	LOG("<ReqQryRCAMSInvestorCombPosition>\n");
	if (pQryRCAMSInvestorCombPosition)
	{
		LOG("\tBrokerID [%s]\n", pQryRCAMSInvestorCombPosition->BrokerID);
		LOG("\tInvestorID [%s]\n", pQryRCAMSInvestorCombPosition->InvestorID);
		LOG("\tInstrumentID [%s]\n", pQryRCAMSInvestorCombPosition->InstrumentID);
		LOG("\tCombInstrumentID [%s]\n", pQryRCAMSInvestorCombPosition->CombInstrumentID);
	}
	LOG("\tnRequestID [%d]\n", nRequestID);
	LOG("</ReqQryRCAMSInvestorCombPosition>\n");

	return m_pApi->ReqQryRCAMSInvestorCombPosition(pQryRCAMSInvestorCombPosition, nRequestID);
};


int CTraderApi::ReqQryInvestorProdRCAMSMargin(CThostFtdcQryInvestorProdRCAMSMarginField* pQryInvestorProdRCAMSMargin, int nRequestID)
{
	LOG("<ReqQryInvestorProdRCAMSMargin>\n");
	if (pQryInvestorProdRCAMSMargin)
	{
		LOG("\tBrokerID [%s]\n", pQryInvestorProdRCAMSMargin->BrokerID);
		LOG("\tInvestorID [%s]\n", pQryInvestorProdRCAMSMargin->InvestorID);
		LOG("\tCombProductID [%s]\n", pQryInvestorProdRCAMSMargin->CombProductID);
		LOG("\tProductGroupID [%s]\n", pQryInvestorProdRCAMSMargin->ProductGroupID);
	}
	LOG("\tnRequestID [%d]\n", nRequestID);
	LOG("</ReqQryInvestorProdRCAMSMargin>\n");

	return m_pApi->ReqQryInvestorProdRCAMSMargin(pQryInvestorProdRCAMSMargin, nRequestID);
};


int CTraderApi::ReqQryRULEInstrParameter(CThostFtdcQryRULEInstrParameterField* pQryRULEInstrParameter, int nRequestID)
{
	LOG("<ReqQryRULEInstrParameter>\n");
	if (pQryRULEInstrParameter)
	{
		LOG("\tExchangeID [%s]\n", pQryRULEInstrParameter->ExchangeID);
		LOG("\tInstrumentID [%s]\n", pQryRULEInstrParameter->InstrumentID);
	}
	LOG("\tnRequestID [%d]\n", nRequestID);
	LOG("</ReqQryRULEInstrParameter>\n");

	return m_pApi->ReqQryRULEInstrParameter(pQryRULEInstrParameter, nRequestID);
};


int CTraderApi::ReqQryRULEIntraParameter(CThostFtdcQryRULEIntraParameterField* pQryRULEIntraParameter, int nRequestID)
{
	LOG("<ReqQryRULEIntraParameter>\n");
	if (pQryRULEIntraParameter)
	{
		LOG("\tExchangeID [%s]\n", pQryRULEIntraParameter->ExchangeID);
		LOG("\tProdFamilyCode [%s]\n", pQryRULEIntraParameter->ProdFamilyCode);
	}
	LOG("\tnRequestID [%d]\n", nRequestID);
	LOG("</ReqQryRULEIntraParameter>\n");

	return m_pApi->ReqQryRULEIntraParameter(pQryRULEIntraParameter, nRequestID);
};


int CTraderApi::ReqQryRULEInterParameter(CThostFtdcQryRULEInterParameterField* pQryRULEInterParameter, int nRequestID)
{
	LOG("<ReqQryRULEInterParameter>\n");
	if (pQryRULEInterParameter)
	{
		LOG("\tExchangeID [%s]\n", pQryRULEInterParameter->ExchangeID);
		LOG("\tLeg1ProdFamilyCode [%s]\n", pQryRULEInterParameter->Leg1ProdFamilyCode);
		LOG("\tLeg2ProdFamilyCode [%s]\n", pQryRULEInterParameter->Leg2ProdFamilyCode);
		LOG("\tCommodityGroupID [%d]\n", pQryRULEInterParameter->CommodityGroupID);
	}
	LOG("\tnRequestID [%d]\n", nRequestID);
	LOG("</ReqQryRULEInterParameter>\n");

	return m_pApi->ReqQryRULEInterParameter(pQryRULEInterParameter, nRequestID);
};


int CTraderApi::ReqQryInvestorProdRULEMargin(CThostFtdcQryInvestorProdRULEMarginField* pQryInvestorProdRULEMargin, int nRequestID)
{
	LOG("<ReqQryInvestorProdRULEMargin>\n");
	if (pQryInvestorProdRULEMargin)
	{
		LOG("\tExchangeID [%s]\n", pQryInvestorProdRULEMargin->ExchangeID);
		LOG("\tBrokerID [%s]\n", pQryInvestorProdRULEMargin->BrokerID);
		LOG("\tInvestorID [%s]\n", pQryInvestorProdRULEMargin->InvestorID);
		LOG("\tProdFamilyCode [%s]\n", pQryInvestorProdRULEMargin->ProdFamilyCode);
		LOG("\tCommodityGroupID [%d]\n", pQryInvestorProdRULEMargin->CommodityGroupID);
	}
	LOG("\tnRequestID [%d]\n", nRequestID);
	LOG("</ReqQryInvestorProdRULEMargin>\n");

	return m_pApi->ReqQryInvestorProdRULEMargin(pQryInvestorProdRULEMargin, nRequestID);
};


int CTraderApi::ReqQryInvestorPortfSetting(CThostFtdcQryInvestorPortfSettingField* pQryInvestorPortfSetting, int nRequestID)
{
	LOG("<ReqQryInvestorPortfSetting>\n");
	if (pQryInvestorPortfSetting)
	{
		LOG("\tExchangeID [%s]\n", pQryInvestorPortfSetting->ExchangeID);
		LOG("\tBrokerID [%s]\n", pQryInvestorPortfSetting->BrokerID);
		LOG("\tInvestorID [%s]\n", pQryInvestorPortfSetting->InvestorID);
	}
	LOG("\tnRequestID [%d]\n", nRequestID);
	LOG("</ReqQryInvestorPortfSetting>\n");

	return m_pApi->ReqQryInvestorPortfSetting(pQryInvestorPortfSetting, nRequestID);
};


int CTraderApi::ReqQryInvestorInfoCommRec(CThostFtdcQryInvestorInfoCommRecField* pQryInvestorInfoCommRec, int nRequestID)
{
	LOG("<ReqQryInvestorInfoCommRec>\n");
	if (pQryInvestorInfoCommRec)
	{
		LOG("\tInvestorID [%s]\n", pQryInvestorInfoCommRec->InvestorID);
		LOG("\tInstrumentID [%s]\n", pQryInvestorInfoCommRec->InstrumentID);
		LOG("\tBrokerID [%s]\n", pQryInvestorInfoCommRec->BrokerID);
	}
	LOG("\tnRequestID [%d]\n", nRequestID);
	LOG("</ReqQryInvestorInfoCommRec>\n");

	return m_pApi->ReqQryInvestorInfoCommRec(pQryInvestorInfoCommRec, nRequestID);
};


int CTraderApi::ReqQryCombLeg(CThostFtdcQryCombLegField* pQryCombLeg, int nRequestID)
{
	LOG("<ReqQryCombLeg>\n");
	if (pQryCombLeg)
	{
		LOG("\tLegInstrumentID [%s]\n", pQryCombLeg->LegInstrumentID);
	}
	LOG("\tnRequestID [%d]\n", nRequestID);
	LOG("</ReqQryCombLeg>\n");

	return m_pApi->ReqQryCombLeg(pQryCombLeg, nRequestID);
};


int CTraderApi::ReqOffsetSetting(CThostFtdcInputOffsetSettingField* pInputOffsetSetting, int nRequestID)
{
	LOG("<ReqOffsetSetting>\n");
	...
	return m_pApi->ReqOffsetSetting(pInputOffsetSetting, nRequestID);
};


int CTraderApi::ReqCancelOffsetSetting(CThostFtdcInputOffsetSettingField* pInputOffsetSetting, int nRequestID)
{
	LOG("<ReqCancelOffsetSetting>\n");
	if (pInputOffsetSetting)
	{
		LOG("\tBrokerID [%s]\n", pInputOffsetSetting->BrokerID);
		LOG("\tInvestorID [%s]\n", pInputOffsetSetting->InvestorID);
	...
	return m_pApi->ReqCancelOffsetSetting(pInputOffsetSetting, nRequestID);
};


int CTraderApi::ReqQryOffsetSetting(CThostFtdcQryOffsetSettingField* pQryOffsetSetting, int nRequestID)
{
	LOG("<ReqQryOffsetSetting>\n");
	if (pQryOffsetSetting)
	{
		LOG("\tBrokerID [%s]\n", pQryOffsetSetting->BrokerID);
		LOG("\tInvestorID [%s]\n", pQryOffsetSetting->InvestorID);
		LOG("\tProductID [%s]\n", pQryOffsetSetting->ProductID);
		LOG("\tOffsetType [%c]\n", pQryOffsetSetting->OffsetType);
	}
	LOG("\tnRequestID [%d]\n", nRequestID);
	LOG("</ReqQryOffsetSetting>\n");

	return m_pApi->ReqQryOffsetSetting(pQryOffsetSetting, nRequestID);
};
