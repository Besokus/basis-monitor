...
public:
	...
	void GetFrontInfo()
	{
		CThostFtdcFrontInfoField FrontInfo;
		memset(&FrontInfo, 0, sizeof(CThostFtdcFrontInfoField));
		m_pUserApi->GetFrontInfo(&FrontInfo);
		printf("%s\n", FrontInfo.FrontAddr);
		printf("%d\n", FrontInfo.FTDPkgFreq);
		printf("%d\n", FrontInfo.QryFreq);
	}
	...
	CombOffsetFlag:LOG("请输入开平方向\t1.开仓\t2.平仓\t3.强平\t4.平今\t5.平昨\t6.强减\t7.本地强平\n");
		...
		ord.VolumeCondition = THOST_FTDC_VC_MV;///最小数量
		ord.MinVolume = 3;
	...
	{
		int new_limitprice;
		LOG("请输入成交价格limitprice：\n");
		cin >> new_limitprice;

		int new_StopPrice;
		LOG("请输入止损价格stopprice：\n");
		cin >> new_StopPrice;
		...
		strcpy_s(ord.ExchangeID, g_chExchangeID);
		int a = m_pUserApi->ReqOrderInsert(&ord, 1);
		LOG((a == 0) ? "报单录入请求限价止损单......发送成功\n" : "报单录入请求限价止损单......发送失败，序号=[%d]\n", a);
	}
	///大商所市价止损单
	void ReqOrderInsert_Touch()
	{
		...
		strcpy_s(ord.ExchangeID, g_chExchangeID);
		int a = m_pUserApi->ReqOrderInsert(&ord, 1);
		LOG((a == 0) ? "报单录入请求市价止损单......发送成功\n" : "报单录入请求市价止损单......发送失败，序号=[%d]\n", a);
	}
	...
	void ReqOffsetSetting()
	{
		...
		strcpy_s(a.InvestorID, g_chInvestorID);
		strcpy_s(a.InstrumentID, g_chInstrumentID);
		//strcpy_s(a.UnderlyingInstrID, g_chInstrumentID);
		//strcpy_s(a.ProductID, g_chProductID);
		a.OffsetType = THOST_FTDC_OT_OPT_OFFSET;
		a.Volume = 1;
		a.IsOffset = 1;
		strcpy_s(a.UserID, g_chUserID);
		strcpy_s(a.ExchangeID, g_chExchangeID);
		int ab = m_pUserApi->ReqOffsetSetting(&a, nRequestID++);
		LOG((ab == 0) ? "对冲设置请求......发送成功\n" : "对冲设置请求......发送失败，序号=[%d]\n", ab);
	}
	...
	void ReqCancelOffsetSetting()
	{
		...
		a.OffsetType = THOST_FTDC_OT_OPT_OFFSET;
		strcpy_s(a.ExchangeID, g_chExchangeID);
		int ab = m_pUserApi->ReqCancelOffsetSetting(&a, nRequestID++);
		LOG((ab == 0) ? "对冲设置撤销请求......发送成功\n" : "对冲设置撤销请求......发送失败，序号=[%d]\n", ab);
	}

	///投资者对冲设置查询
	void ReqQryOffsetSetting()
	{
		CThostFtdcQryOffsetSettingField a = { 0 };
	}
	...
	void ReqQryCombLeg()
	{
		CThostFtdcQryCombLegField a = { 0 };
		strcpy_s(a.LegInstrumentID, g_chInstrumentID);
		int b = m_pUserApi->ReqQryCombLeg(&a, nRequestID++);
	}
	...
	int bankid = 0;
		...
		strcpy_s(a.Password, "yangweitao_654321");///期货密码--资金密码
		//a.InstallID = 1;///安装编号
	...
	void ReqFromFutureToBankByFuture()
	{
		...
		a.RequestID = 0;///请求编号
		a.TID = 0;///交易ID
		strcpy_s(a.Digest, "test_yang");///摘要
